...
// Gathering this is non-trivial, so it's nice to do it in one place
...
type appResources struct {
	MemoryLimit         *uint64
	CPUQuota            *uint64 // The CPU quota as a percent (100 = 1 core)
	LinuxCPUShares      *uint64
	LinuxOOMScoreAdjust *int
}
...
func prepareApp(p *stage1commontypes.Pod, ra *schema.RuntimeApp) (*preparedApp, error) {
	...
	pa.env.Set("AC_APP_NAME", pa.name)
	...
	pa.noNewPrivileges = getAppNoNewPrivileges(ra.App.Isolators)

	// Determine mounts
	pa.mounts, err = GenerateMounts(ra, p.Manifest.Volumes, ConvertedFromDocker(pa.imageManifest))
	if err != nil {
		return nil, errwrap.Wrap(errors.New("unable to compute mounts"), err)
	}

	// Compute resources
	pa.resources, err = getAppResources(ra.App.Isolators)
	if err != nil {
...
func getAppResources(isolators types.Isolators) (appResources, error) {
	...
	withIsolator := func(name string, f func() error) error {
		...
		if err != nil {
			return errwrap.Wrap(errors.New("could not check for isolator "+name), err)
		}
...
// XXX(cdc) rename this
func (pa *preparedApp) write(p *stage1commontypes.Pod) error {
