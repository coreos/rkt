...
func (e *podEnv) setupNets(nets []activeNet) error {
	...
	_, ex := os.Stat(resolvPath)
	hasResolvConf := !os.IsNotExist(ex)
	...
	for i, n = range nets {
		...
		// DNS response will win, unless `--dns` was passed to stage0
		if n.runtime.HasDNSConfig() {
			if !hasResolvConf {
				f, err := os.Create(resolvPath)
				if err != nil {
					return errwrap.Wrap(fmt.Errorf("error creating resolv.conf"), err)
				}
				_, err = f.WriteString(makeResolvConf(n.runtime.DNS, n.conf.Name))
				if err != nil {
					return errwrap.Wrap(fmt.Errorf("failed to write resolv.conf"), err)
				}
				err = f.Close()
				if err != nil {
					return errwrap.Wrap(fmt.Errorf("failed to write resolv.conf"), err)
...
// Generates resolv.conf contents given a cni DNS configuration
func makeResolvConf(d cnitypes.DNS, pluginName string) string {
	content := fmt.Sprintf("# Generated by rkt from network %s\n\n", pluginName)
